...
parser.add_argument('--batch_size', default=4, type=int,
                    help='Size of batch to evaluate')
...
def test_net(save_folder, net, cuda, dataset,batch_size,transform, top_k,
    ...
    max_iterations = len(dataset) // args.batch_size
    if args.cuda:
...
#         print(h)
#         print(w)
#         print(np.sum(np.abs(images[2].numpy()-im.numpy())))
        if args.cuda:
            x = Variable(images.cuda())
#             y = Variable(im.unsqueeze(0).cuda())
        else:
            x = Variable(images)
        _t['im_detect'].tic()
#         print(i)
...
#                 mask = np.tile(np.greater(dets[:,0],0),(5,1)).T
                dets = torch.masked_select(dets, mask).view(-1, 5)               
#                 dets = np.reshape(dets[mask],[-1,5])
#                 if dets.shape[0] == 0:
#                     continue
#                 mask = dets[:, 0].gt(0.).expand(5, dets.size(0)).t()
                if dets.size(0) == 0:
                ...
                cls_dets = np.hstack((boxes.cpu().numpy(),
                                      scores[:, np.newaxis])).astype(np.float32,
                                                                     copy=False)
                all_boxes[j][(batch_size) * i + b] = cls_dets
        for b in range(batch_size):
            print('im_detect: {:d}/{:d} {:.3f}s'.format(i*batch_size + b + 1,
                                                num_images, detect_time/batch_size))
...
def main():
    ...
    dataset = StanfordDetection(STANFORD_ROOT, [set_type],
                           BaseTransform(cfg['width'],cfg['height'], dataset_mean),
                           StanfordAnnotationTransform())
    if args.cuda:
        net = net.cuda()
        cudnn.benchmark = True
    # evaluation
    test_net(args.save_folder, net, args.cuda, dataset,args.batch_size,
             BaseTransform(cfg['width'],cfg['height'], dataset_mean), args.top_k, 1200,
